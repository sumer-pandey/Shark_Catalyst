...
MYTHS = [
...

    {
        ...
        "check_sql": """
            SELECT
              SUM(CASE WHEN Has_Patents = 1 THEN 1 ELSE 0 END) AS total_patent,
              SUM(CASE WHEN Has_Patents = 1 AND invested_amount IS NOT NULL THEN 1 ELSE 0 END) AS funded_patent,
              SUM(CASE WHEN Has_Patents = 0 THEN 1 ELSE 0 END) AS total_no_patent,
...

    {
        ...
        "check_sql": """
            SELECT
              SUM(CASE WHEN bootstrapped=1 THEN 1 ELSE 0 END) AS total_boot,
              SUM(CASE WHEN bootstrapped=1 AND invested_amount IS NOT NULL THEN 1 ELSE 0 END) AS funded_boot,
              SUM(CASE WHEN bootstrapped=0 THEN 1 ELSE 0 END) AS total_nonboot,
...
def page_myths(filters):
    ...
    df_check = None
    try:
        df_check = cached_query(myth['check_sql'], {"season": season})
