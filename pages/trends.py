...
def page_trends(filters):
    ...
    """
    try:
        df_kpis = cached_query(sql_season_kpis, {"season": season})
    ...
    try:
        ...
        """
        df_ss = cached_query(sql_sector_season, {"season": season})
    ...
    try:
        ...
        GROUP BY
          ...
          END;
        """
        df_ps = cached_query(sql_prod_service, {"season": season})
        if df_ps is None or df_ps.empty:
            st.info("Product/service mapping returned no rows.")
        else:
            # user-friendly table
            df_ps_display = df_ps.copy()
            df_ps_display['total_invested'] = df_ps_display['total_invested'].apply(lambda x: format_lakhs(x))
            df_ps_display['avg_ticket'] = df_ps_display['avg_ticket'].apply(lambda x: format_lakhs(x) if pd.notnull(x) else "-")
            df_ps_display = df_ps_display.rename(columns={
                'product_service':'Type',
                'deals_count':'Deals',
                'total_invested':'Total invested (L)',
                'avg_ticket':'Avg ticket (L)'
            })
            df_ps_display.index = range(1, len(df_ps_display) + 1)
            fig = px.pie(df_ps_display, names='Type', values='Deals', title='Product vs Service share (by deals)')
    ...
    try:
        ...
        if inv_raw is None or inv_raw.empty:
            ...
            # explode combined investor strings into individual names
            rows = []
            ...
            inv_total['Total invested (L)'] = inv_total['total_invested'].apply(lambda x: float(x))
            ...
            # Plot total invested
            fig1 = px.bar(inv_total, x='investor', y='total_invested', title='Top investors by Total invested (L)')
            fig1.update_traces(marker_color=ACCENT)
            fig1.update_layout(xaxis_tickangle=-45, height=420, yaxis_title='Total invested (L)', paper_bgcolor=BG, plot_bgcolor=BG, font=dict(color=TEXT))
    ...
    try:
        ...
        if dd is None or dd.empty:
            ...
            with col_violin:
                fig_v = px.violin(dd, y='invested_amount', box=True, points='all', title='Ticket distribution (violin)')
                fig_v.update_layout(yaxis_title='Invested amount (L)', height=420, paper_bgcolor=BG, plot_bgcolor=BG, font=dict(color=TEXT))
